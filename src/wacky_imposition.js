...
export class WackyImposition {
  ...
  build_1_3rd_lineFunction() {
    return (info) => {
      const vGap = (row) => {
        return info.isPacked ? info.gap[1] : info.gap[1] * row;
      };
      const hGap = (col) => {
        return info.isPacked ? info.gap[0] : info.gap[0] * col;
      };
      let foldMarks = [];
      [0, 1, 2, 3].forEach((row) => {
        [0, 1, 2].forEach((page) => {
          foldMarks = foldMarks.concat(
            this.crosshairMark(
  ...
  build_3_3_4_sheetList(pageCount) {
    ...
    const f = this.flipPage;
    const sheets = [];
    const sheetCount = Math.ceil(pageCount / 40.0);
    console.log(
      `Building the 3/3/4 pages. Given ${pageCount} page count, there will be ${sheetCount} sheets...`
    );
    for (let sheet = 0; sheet < sheetCount; ++sheet) {
      const i = sheet * 40 - 1;
      const frontThrees = [
      ...
      console.log(
        `Sheet ${sheet} has front 3s ${frontThrees} and front 4 ${frontFour}, w/ back 3s ${backThrees} and back 4',${backFour}`
      );
  ...
  build_3_3_4_lineFunction() {
    return (info) => {
      ...
      const cutBetweenTheThrees = {
        start: {
          x: hGap(2) + 2 * info.renderPageSize[0],
          y: info.renderPageSize[1] * 2 + info.gap[1],
        },
        end: { x: hGap(2) + 2 * info.renderPageSize[0], y: info.paperSize[1] },
        thickness: 0.25,
        opacity: 0.4,
      };
      const foldBetweenTheFours = {
        start: {
          x: hGap(2) + 2 * info.renderPageSize[0],
          y: info.renderPageSize[1] * 2 + info.gap[1],
        },
  ...
  build_6_10s_lineFunction() {
    return (info) => {
      const cutOffset = !this.duplex || info.isFront ? 4 : 6;
      const vGap = (row) => {
        return info.isPacked ? info.gap[1] : info.gap[1] * (2 * row);
      };
      const hGap = (col) => {
        return info.isPacked ? info.gap[0] : col * info.gap[0];
      };
      const baseCuts = [
  ...
  build_3_6s_sheetList(pageCount) {
    ...
    const frontFunc = (i) => {
      return [page(i + 2), page(i + 11), page(i + 10), page(i + 3), page(i + 6), page(i + 7)];
    };
    const backFunc = (i) => {
      return [page(i + 8), page(i + 5), page(i + 4), page(i + 9), page(i + 12), page(i + 1)];
    };
  ...
  build_strips_sheetList(rows, folioPerRow, pageCount, frontPageFunc, backPageFunc) {
    ...
    const backs = [];
    // const page = this.page;
    const blank = this.blankPage;
    const totalPagesPerRow = folioPerRow * 4;
    const rowCount = Math.ceil(pageCount / totalPagesPerRow);
    console.log(
      `Building the ${rows} rows of ${folioPerRow * 2} pages. Given ${pageCount} page count, there will be ${rowCount} rows...`
    );
    for (let row = 0; row < rowCount; ++row) {
      const i = row * totalPagesPerRow - 1;
      const front = frontPageFunc(i);
      const back = backPageFunc(i);
      if (!this.duplex) {
      ...
      console.log(
        `   -> adding front  ${fronts[fronts.length - 1]} and back ${backs[fronts.length - 1]}`
      );
    }
    const sheets = [];
    for (let row = 0; row < rowCount; ++row) {
      const sheet = Math.floor(row / rows);
      if (row % rows == 0) {
        sheets[sheet * 2] = [];
        sheets[sheet * 2 + 1] = [];
        console.log(sheets);
      ...
      console.log(
        ` -> row ${row} => sheet ${sheet} grabs front ${fronts[row]} and back ${backs[row]}`
      );
    }
    if (sheets[sheets.length - 1].length < rows) {
      const rowOfBlanks = new Array(folioPerRow * 2);
      for (let j = 0; j < rowOfBlanks.length; ++j) {
        rowOfBlanks[j] = blank();
      }
      console.log(`I present you my blanks! [for ${folioPerRow}] : ${rowOfBlanks}`);
      for (let filler = 0; filler < rows - (rowCount % rows); ++filler) {
        const sheet = Math.floor(rowCount / rows);
        if (filler % 2 == 0) {
    ...
    for (let i = 0; i < sheets.length; ++i) {
      if (i % 2 == 1 && !this.duplex) {
        // stupid "flip on short edge" rotation plans...
        sheets[i].reverse();
  ...
  build_2_16s_sheetList(pageCount) {
    ...
    const f = this.flipPage;
    const sheets = [];
    const sheetCount = Math.ceil(pageCount / 32.0);
    console.log(
      `Building the 32 pages. Given ${pageCount} page count, there will be ${sheetCount} sheets...`
    );
    for (let sheet = 0; sheet < sheetCount; ++sheet) {
      const i = sheet * 32 - 1;
      const front = [
