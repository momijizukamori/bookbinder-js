...
const generatePageInfo = (page) => {
  return { info: page, isSigEnd: true, isSigStart: true };
};
...
describe('PerfectBound model', () => {
  ...
  it('correctly orders folio pages with no duplex', () => {
    ...
    const expected = {
      ...
      sigconfig: ['N/A'],
      pagelistdetails: [
        [3, 2, 7, 6, 11, 10, 15, 14, 19, 18, 23, 22, 27, 26, 31, 30].map(generatePageInfo),
        [1, 4, 5, 8, 9, 12, 13, 16, 17, 20, 21, 24, 25, 28, 29, 32].map(generatePageInfo),
      ],
    };
  ...
  it('correctly orders quarto pages with no duplex', () => {
    ...
    const expected = {
      ...
      sigconfig: ['N/A'],
      pagelistdetails: [
        [4, 1, 7, 6, 12, 9, 15, 14, 20, 17, 23, 22, 28, 25, 31, 30].map(generatePageInfo),
        [8, 5, 3, 2, 16, 13, 11, 10, 24, 21, 19, 18, 32, 29, 27, 26].map(generatePageInfo),
      ],
    };
